      ...
      if (isFile)
        {
          ...
          if (gst_uri_handler_get_uri_type (GST_URI_HANDLER (pel))
              == GST_URI_SRC)
            path = QFileDialog::getOpenFileName (NULL,
                                                 "Open Media Source File...",
                                                 dir);
          else
            path = QFileDialog::getSaveFileName (NULL, "Open Sink File...",
                                                 dir);
      ...
      for (std::size_t j = 0; j < res[i].m_pads.size (); j++)
        {
          ...
          if (peerPad)
            {
              GstElement *peerElement = GST_ELEMENT (
                  gst_pad_get_parent (peerPad));
  ...
  if (!gst_element_factory_can_sink_all_caps (destFactory, srcCaps))
    {
      ...
      qDebug () << "The dest element " << destName
          << " can not sink this caps: " << caps_string;
      g_free (caps_string);
      goto done;
    }
