      ...
      if (child.toElement ().tagName () == "property")
        {
          ...
          if (!param)
            {
              gchar *elementName = gst_element_get_name (element);
              qDebug () << "problem with setting property `" << name
                  << "` for `" << elementName << "`";
              g_free (elementName);
              continue;
            }
          ...
          if (info[i].m_connections[j].m_elementId != (size_t) -1
              && info[i].m_connections[j].m_padId != (size_t) -1)
            {
              ...
              for (elementPos = 0; elementPos < info.size (); elementPos++)
                {
                  if (info[elementPos].m_id
                      == info[i].m_connections[j].m_elementId)
                    {
                      for (padPos = 0; padPos < info[elementPos].m_pads.size ();
                          padPos++)
              ...
              if (elementPos < info.size ()
                  && padPos < info[elementPos].m_pads.size ())
                {
                  ...
                  xmlWriter.writeAttribute (
                      "pad-name",
                      info[elementPos].m_pads[padPos].m_name.c_str ());
                  xmlWriter.writeEndElement ();
                }
            }
                          ...
                          if (!isExists)
                            {
                              ...
                              newConnetion.element1 =
                                  elNode.attribute ("name").toStdString ();
                              newConnetion.element2 =
                                  padChild.toElement ().attribute (
                                      "element-name").toStdString ();
                              newConnetion.pad1 =
              ...
              if (pad1 && pad2)
                {
                  if (GST_PAD_IS_SRC (pad1))
                    gst_element_link_pads (el1, connections[i].pad1.c_str (),
                                           el2, connections[i].pad2.c_str ());
                  else
                    gst_element_link_pads (el2, connections[i].pad2.c_str (),
                                           el1, connections[i].pad1.c_str ());
