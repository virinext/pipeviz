          ...
          if (GST_IS_ELEMENT_FACTORY (feature))
            {
              ...
              int rank = gst_plugin_feature_get_rank (
...
PluginsListDialog::PluginsListDialog (PluginsList* pluginList, QWidget *pwgt,
                                      Qt::WindowFlags f) :
    QDialog (pwgt, f), m_pPluginsList (pluginList), m_pGraph (NULL)
{
  ...
  if (release_date != NULL)
    {
      ...
      descr += "<b>Source release date</b>: " + QString (str) + " "
          + QString (tz) + "<br>";
      g_free (str);
    }
